...
usage() {
  cat <<EOF
Usage:
  bash iac/deploy-instance.sh <instance-name> [terraform-apply-args...]
...
if ! terraform -chdir="${SCRIPT_DIR}" workspace select "${INSTANCE_NAME}" >/dev/null 2>&1; then
  terraform -chdir="${SCRIPT_DIR}" workspace new "${INSTANCE_NAME}" >/dev/null
fi

echo "Running terraform apply for workspace ${INSTANCE_NAME}..."
terraform -chdir="${SCRIPT_DIR}" apply -auto-approve "$@"

echo
echo "Deployment complete for instance ${INSTANCE_NAME}. Key outputs:"
